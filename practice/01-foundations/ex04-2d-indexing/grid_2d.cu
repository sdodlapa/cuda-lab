 ...
 * 
 * Work with 2D data using 2D thread grids.
 * 
...
// TODO 1: Write a 2D kernel that scales each matrix element
// output[row][col] = input[row][col] * scalar
// Remember: 2D array is stored as 1D in memory (row-major)
__global__ void matrixScale(float *input, float *output, 
                            float scalar, int width, int height) {
    // TODO: Calculate column (x) and row (y) from thread indices
    int col = 0;  // Fix this!
    int row = 0;  // Fix this!
    
    // TODO: Check bounds (col < width && row < height)
    
    // TODO: Calculate 1D index from 2D coordinates
    // index = row * width + col
    
    // TODO: Perform the scaling operation
...
int main() {
    ...
    // TODO 2: Set up 2D block and grid dimensions
    // Use 16x16 threads per block (256 total)
    dim3 threadsPerBlock(16, 16);
    
    // Calculate grid dimensions to cover the entire matrix
    // Use ceiling division: (size + blockSize - 1) / blockSize
    dim3 blocksPerGrid(1, 1);  // TODO: Fix this!
    ...
    // TODO 3: Launch the kernel with 2D configuration
